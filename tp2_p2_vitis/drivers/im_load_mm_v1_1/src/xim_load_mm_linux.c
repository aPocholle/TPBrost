...
typedef struct {
    ...
    XIm_load_mm_uio_map maps[ MAX_UIO_MAPS ];
} XIm_load_mm_uio_info;

/***************** Variable Definitions **************************************/
static XIm_load_mm_uio_info uio_info;
...
static int uio_info_read_name(XIm_load_mm_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XIm_load_mm_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XIm_load_mm_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XIm_load_mm_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XIm_load_mm_Initialize(XIm_load_mm *InstancePtr, const char* InstanceName) {
	XIm_load_mm_uio_info *InfoPtr = &uio_info;
